...
transformed parameters {
  ...
  vector<lower=0, upper=20>[nSubjects] shift;

  for (i in 1:nSubjects) {
    
    m[i] = Phi_approx(mu_pr[1] + sigma[1] * m_pr[i]) * 20;
    shift[i] = Phi_approx(mu_pr[2] + sigma[2] * shift_pr[i]) * 20;
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials[s]) {
      ...
      // prob[2] = (1)/(1 + exp((-(m[s]-10) )*( (( dot_product(prevSelf[s,1:nTrain[s]],PS)/sum(prevSelf[s,1:nTrain[s]]) ) )  - (shift[s]) ) )) ;
      prob[2] = (1)/(1 + exp((-(m[s]-10) )*( (( ( sg[s,t] ) - mean(sg[s,]) ) / sd(sg[s,]) )  - (shift[s]-10) ) )) ;
      // print("Probabilities")
      // print(prob)
      // print(sum(prob))
      prob[1] = 1 - prob[2];
...
generated quantities {
  ...
  real<lower=0, upper=20> mu_shift;
  ...
  mu_m = Phi_approx(mu_pr[1]) * 20;
  mu_shift = Phi_approx(mu_pr[2]) * 20;
