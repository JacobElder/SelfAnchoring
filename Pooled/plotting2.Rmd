...
pos_adj <- read.csv(here("Pooled/input/adjacencyMatrix_p.csv"))
colnames(pos_adj) <- 1:148
pos_adj <- as.matrix(pos_adj)
#pos_att <- read.csv("input/traitLevelAttribute_p.csv")
pos_att <- read.csv(here("Pooled/input/allPosCents.csv"))
...
# Max homophily
i <- indDiffs3$subID[which(indDiffs3$groupHomop==max(indDiffs3$groupHomoph,na.rm=T))]
i <- indDiffs$subID[which(indDiffs3$groupHomoph==-sort(-indDiffs3$groupHomoph)[2])]
V(posGraph)$C <- testSub$ingChoice[order(testSub$Idx)]
cleanTest$groupHomoph[cleanTest$subID==i] <- netseg::assort(posGraph, "C")

V(posGraph)$C <- as.numeric(V(posGraph)$C)
graph_pos2 <- intergraph::asNetwork(posGraph)
graph_pos2 %v% "C" = as.character(V(posGraph)$C)

set.seed(5)

p1 <- ggnet2(graph_pos2, size = "outdegree",  color = "C", palette = "Dark2", mode = "kamadakawai", directed=TRUE, layout.exp = .65, alpha = .7, edge.alpha = .25) +
  guides(color = FALSE, size = FALSE)
p1
```
...
ghforplot2 <- indDiffs2 %>% select(groupHomoph, seHomoph, SStatus:IdImp, Therm_1, Therm_2) %>% correlation(bayesian=T) %>% filter(Parameter1 == "groupHomoph" | Parameter2 == "groupHomoph") %>% arrange(rho)
ghforplot2
bayesCorPlot(ghforplot2)

ghforplot3 <- indDiffs3 %>% select(groupHomoph, SStatus:IdImp, -Condition, -mixed, -racid2, Therm_1, Therm_2) %>% correlation(bayesian=T) %>% filter(Parameter1 == "groupHomoph" | Parameter2 == "groupHomoph") %>% arrange(rho)
ghforplot3
bayesCorPlot(ghforplot3)


NTB.GH.Scatter1 <- ggplot(indDiffs1, aes(x=NTB,y=groupHomoph)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Need to Belong") + ylab("Trait Segregation")

NTB.GH.Scatter2 <- ggplot(indDiffs2, aes(x=NTB,y=groupHomoph)) + geom_point() + geom_smooth(method="lm",color="black") + jtools::theme_apa() + xlab("Need to Belong") + ylab("Trait Segregation") #+ 
#  scale_color_manual( 
#                      labels=c("Negation","Lower Status","Higher Status"), values = wes_palette("Darjeeling1"))

NTB.GH.Scatter3 <- ggplot(indDiffs3, aes(x=NTB,y=groupHomoph)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Need to Belong") + ylab("Trait Segregation") #+ scale_color_manual(labels = c("Majority","Minority"), values = c("#44AA99","#332288")) + scale_fill_manual( 
#                      labels=c("Majority","Minority"), values = c("#44AA99","#332288"))

NTB.GH.Scatter <- NTB.GH.Scatter1 + NTB.GH.Scatter2 + NTB.GH.Scatter3
NTB.GH.Scatter
ggsave(file = "~/Desktop/NTB.GH.scatter.png", dpi = 300, units = "in", width= 16, height = 5)

RSE.GH.Scatter1 <- ggplot(indDiffs1, aes(x=RSE,y=groupHomoph)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Self-Esteem") + ylab("Trait Segregation")

RSE.GH.Scatter2 <- ggplot(indDiffs2, aes(x=RSE,y=groupHomoph)) + geom_point() + geom_smooth(method="lm",color="black") + jtools::theme_apa() + xlab("Self-Esteem") + ylab("Trait Segregation") #+ 
#  scale_color_manual( 
#                      labels=c("Negation","Lower Status","Higher Status"), values = wes_palette("Darjeeling1"))

RSE.GH.Scatter3 <- ggplot(indDiffs3, aes(x=RSE,y=groupHomoph)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Self-Esteem") + ylab("Trait Segregation") #+ scale_color_manual(labels = c("Majority","Minority"), values = c("#44AA99","#332288")) + scale_fill_manual( 
#                      labels=c("Majority","Minority"), values = c("#44AA99","#332288"))

RSE.GH.Scatter <- RSE.GH.Scatter1 + RSE.GH.Scatter2 + RSE.GH.Scatter3
RSE.GH.Scatter
ggsave(file = "~/Desktop/RSE.GH.scatter.png", dpi = 300, units = "in", width= 16, height = 5)

NTB.GH.Scatter / RSE.GH.Scatter
ggsave(file = "~/Desktop/RSE.NTB.GH.scatter.png", dpi = 300, units = "in", width= 16, height = 10)

Cent.GH.Scatter2 <- ggplot(indDiffs2, aes(x=MGIS_Cent,y=groupHomoph)) + geom_point() + geom_smooth(method="lm",color="black") + jtools::theme_apa() + xlab("Identity Centrality") + ylab("Trait Segregation")

Cent.GH.Scatter3 <- ggplot(indDiffs3, aes(x=MGIS_Cent,y=groupHomoph)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Identity Centrality") + ylab("Trait Segregation")

Cent.GH.Scatter2 + Cent.GH.Scatter3
ggsave(file = "~/Desktop/Cent.GH.scatter.png", dpi = 300, units = "in", width= 9, height = 5)
```
...
SI.PR.Scatter1 <- ggplot(indDiffsp1, aes(x=SI,y=m)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Social Identification") + ylab("Projection Rate")

SI.PR.Scatter2 <- ggplot(indDiffsp2, aes(x=MGIS,y=m, color=outgroup)) + geom_point() + geom_smooth(method="lm") + jtools::theme_apa() + xlab("Social Identification") + ylab("Projection Rate") + scale_color_manual(labels=c("Higher Status","Lower Status","Negation"), values = wes_palette("Darjeeling1"))

SI.PR.Scatter3 <- ggplot(indDiffsp3, aes(x=MGIS, y=m, color=condition)) + geom_point() + geom_smooth(method="lm") + jtools::theme_apa() + xlab("Social Identification") + ylab("Projection Rate") + scale_color_manual(labels = c("Majority","Minority"), values = c("#44AA99","#332288")) + scale_fill_manual(labels=c("Majority","Minority"), values = c("#44AA99","#332288"))

###

SI.PR.Scatter1 <- ggplot(indDiffsp1, aes(x=SI,y=m)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Social Identification") + ylab("Projection Rate")

SI.PR.Scatter2 <- ggplot(indDiffsp2, aes(x=MGIS,y=m)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Social Identification") + ylab("Projection Rate") + scale_color_manual(labels=c("Higher Status","Lower Status","Negation"), values = c('black','black','black'))

SI.PR.Scatter3 <- ggplot(indDiffsp3, aes(x=MGIS, y=m, color=condition)) + geom_point() + geom_smooth(method="lm", color="black") + jtools::theme_apa() + xlab("Social Identification") + ylab("Projection Rate") + scale_color_manual(labels = c("Majority","Minority"), values=c("black",'black','black'))

SI.PR.Scatter <- SI.PR.Scatter1 + SI.PR.Scatter2 + SI.PR.Scatter3
SI.PR.Scatter
ggsave(file = "~/Desktop/SI.PR.scatter.png", dpi = 300, units = "in", width= 16, height = 5)
```
