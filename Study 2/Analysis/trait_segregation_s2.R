# Trait Segregation Correlations — Study 2
...
cond_col <- intersect(c("condition","outgroup","mgroup"), names(id_df))
id_df <- id_df[, c("subj_idx","subID", cond_col, "groupHomoph", avail_scales)]

# ── 2. Load computational parameters ─────────────────────────────────────────
params_file <- here("Results","params_ind_s2_sym_lambda.csv")
...
loo_path_asym <- here("Fits","loo_s2_asym_lambda.rds")

delta_elpd_df <- NULL
if (file.exists(loo_path_sym) && file.exists(loo_path_asym)) {
  traindf <- read.csv(here("Study 2/Cleaning/output/fullTrain_fixed.csv")) |>
    filter(!is.na(selfResp))
  common_ids  <- sort(intersect(uIds, unique(traindf$subID)))
  fulldf_sub  <- filter(fulldf, subID %in% common_ids)
  uIds_sub    <- sort(common_ids)
  maxTrials   <- max(fulldf_sub$trialTotalT2)
  nTrials_vec <- sapply(uIds_sub, function(id) nrow(filter(fulldf_sub, subID == id)))
  ...
  delta_elpd_vec <- sapply(seq_along(uIds_sub), function(i) {
    idx <- ((i-1)*maxTrials + 1) : ((i-1)*maxTrials + nTrials_vec[i])
    sum(elpd_asym[idx]) - sum(elpd_sym[idx])
  })
  delta_elpd_df <- data.frame(subj_idx = seq_along(uIds_sub), delta_elpd = delta_elpd_vec)
}
...
write.csv(res, here("Results","trait_segregation_s2_correlations.csv"), row.names=FALSE)
message("Saved: Results/trait_segregation_s2_correlations.csv")
