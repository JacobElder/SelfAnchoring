...
```{r}
statusDf <- pivot_longer(indDiffs, cols=ends_with("_Status"), names_to="Outgroup", values_to="Status") %>% select(subID, Outgroup, Status) %>% drop_na()
statusDf$Outgroup <- gsub("_Status","", statusDf$Outgroup)

statusDf$Outgroup <- as.factor(statusDf$Outgroup)
statusDf$subID <- as.factor(statusDf$subID)
statusDf$Outgroup <- relevel(statusDf$Outgroup, ref = "UCR")

m <- lmer( scale(Status) ~ Outgroup +  ( 1  | subID), data = statusDf)
summary(m)
ggpredict(m, c("Outgroup")) %>% plot()

m<-ezANOVA(statusDf, dv = Status, wid = subID, between = as.factor(Outgroup) )
m

ggplot(statusDf, aes(Outgroup, Status)) + 
...
```{r}
warmthDf <- pivot_longer(indDiffs, cols=starts_with("Therm_"), names_to="Outgroup", values_to="Warmth") %>% select(subID, Outgroup, Warmth) %>% drop_na()
warmthDf$Outgroup <- gsub("Therm_1","UCR", warmthDf$Outgroup)
warmthDf$Outgroup <- gsub("Therm_2","UCLA", warmthDf$Outgroup)
warmthDf$Outgroup <- gsub("Therm_4","CSU LA", warmthDf$Outgroup)

warmthDf$Outgroup <- as.factor(warmthDf$Outgroup)
warmthDf$subID <- as.factor(warmthDf$subID)
warmthDf$Outgroup <- relevel(warmthDf$Outgroup, ref = "UCR")

m <- lmer( scale(Warmth) ~ Outgroup +  ( 1  | subID), data = warmthDf)
summary(m)
ggpredict(m, c("Outgroup")) %>% plot()

m<-ezANOVA(warmthDf, dv = Warmth, wid = subID, between = as.factor(Outgroup) )
m

indDiffs$groupHomoph.Z <- scale(indDiffs$groupHomoph)
ggplot(warmthDf, aes(Outgroup, Warmth)) + 
  ggdist::stat_halfeye(adjust = .5, width = .7, .width = 0, justification = -.2, point_colour = NA) + 
  geom_boxplot(width = .2, outlier.shape = NA) + 
  geom_jitter(width = .05, alpha = .3) + labs(y="Perceived Status",x="University") + jtools::theme_apa()
```
...
```{r}
m <- glmer( ingChoiceN ~ desirability.Z * outgroup +  ( desirability.Z  | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ selfResp.Z * outgroup + desirability.Z +  ( selfResp.Z | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup + desirability.Z + ( WSR.Z | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ predicted.Z*outgroup + desirability.Z + ( predicted.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ er.Z*outgroup + desirability.Z + ( er.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ slope.Z*outgroup + desirability.Z + ( slope.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ neighAveAllSE.Z*outgroup + desirability.Z + ( neighAveAllSE.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ neighAveOutSE.Z*outgroup + desirability.Z + ( neighAveOutSE.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- glmer( ingChoiceN ~ neighAveInSE.Z*outgroup + desirability.Z + ( neighAveInSE.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsim)*outgroup + scale(outGsim) + (  scale(inGsim) + scale(outGsim) | subID) + (1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
#        ifelse(fullTest$outgroup == "CSULA", fullTest$InCSULATherm, c(fullTest$InUCLATherm, fullTest$InCSULATherm)/2)
#        )
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(InOutTherm)*outgroup + ( predicted.Z | subID) + ( predicted.Z | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "InOutTherm", "outgroup")) %>% plot()
# 
# fullTest$InOutStatus <- ifelse(fullTest$outgroup == "UCLA", fullTest$InUCLAStatus, 
#        ifelse(fullTest$outgroup == "CSULA", fullTest$InCSULAStatus, c(fullTest$InUCLAStatus, fullTest$InCSULAStatus)/2)
#        )
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(InOutStatus)*outgroup + ( predicted.Z | subID) + ( predicted.Z | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "InOutStatus", "outgroup")) %>% plot()
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(SGO_Self_UCR) + ( predicted.Z | subID) + ( predicted.Z | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "SGO_Self_UCR")) %>% plot()
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(Ent)*outgroup + ( predicted.Z | subID) + ( predicted.Z | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "Ent")) %>% plot()
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(MGIS)*outgroup + ( predicted.Z | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "MGIS")) %>% plot()
# 
# # People who feel less similar to their group self-anchor more??
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(GSim) + ( predicted.Z | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "GSim")) %>% plot()
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(PubCSE) + ( predicted.Z | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "GSim")) %>% plot()
# 
# m <- glmer( as.factor(ingChoiceN) ~ predicted.Z*scale(PrivCSE) + ( predicted.Z | subID) + ( 1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
#                                     optCtrl = list(maxfun = 100000)),
#     nAGQ = 1)
# summary(m)
# ggpredict(m, c("predicted.Z", "GSim")) %>% plot()
