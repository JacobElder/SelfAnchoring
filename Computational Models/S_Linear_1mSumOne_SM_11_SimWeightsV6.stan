data {
  ...
  real inGmean[nSubjects, maxTrials]; // self-weighted similarity
  real outGmean[nSubjects, maxTrials]; // self-weighted similarity
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials[s]) {
      ...
      curW[1] = (wOut[s]-1) * outGmean[s,t]; // multiply weight by summed sim-to-outgroup choices thus far
      curW[2] = (wIn[s]-1) * inGmean[s,t]; // multiply weight by summed sim-to-ingroup choices thus far
...
generated quantities {
  ...
  { // local section, this saves time and space
    ...
    for (t in 1:nTrials[s]) {
      ...
      curW[1] = (wOut[s]-1) * outGmean[s,t];
      curW[2] = (wIn[s]-1) * inGmean[s,t];
