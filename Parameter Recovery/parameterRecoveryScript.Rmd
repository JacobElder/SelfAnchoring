...
sampTemps <- rtruncnorm(maxSubjs, a=.5, b=9.5, mean = 7.5, sd = 1.5)
#sampMs <- runif(iter, min=.1, max=5) # rtruncnorm(iter, a=0, b=10, mean = 5, sd = 1.5)
sampMs <- rtruncnorm(maxSubjs, a=.1, b=5, mean = 1.5, sd = 1.5)
sampBiass <- rtruncnorm(maxSubjs, a=.1, b=.9, mean = .5, sd = .25)

origParams <- data.frame(temp=sample(sampTemps),m=sample(sampMs),bias=sample(sampBiass))
```
