...
transformed parameters {
  ...
  for (i in 1:nSubjects) {
    ...
    m_in[i] = Phi_approx(mu_pr[3]  + sigma[3]  * m_in_pr[i]) * 20;
    m_out[i] = Phi_approx(mu_pr[4]  + sigma[4]  * m_out_pr[i]) * 20;
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    GPin[1:nTrain[s]] = rep_vector(1,nTrain[s])./(1 + exp((-(m_in[s]-10) )*(prevSelf[s,1:nTrain[s]] -  (shift[s]+1) )));
    GPout[1:nTrain[s]] = rep_vector(1,nTrain[s])./(1 + exp((-(m_out[s]-10) )*(prevSelf[s,1:nTrain[s]] - (shift[s]+1) )));
...
generated quantities {
  ...
  mu_m_in   = Phi_approx(mu_pr[3]) * 20;
  mu_m_out = Phi_approx(mu_pr[4]) * 20;
