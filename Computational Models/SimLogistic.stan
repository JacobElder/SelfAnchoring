...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    GPin[1:nTrain[s]] = rep_vector(1,nTrain[s])./(1 + exp(-(m_in[s]-1)*(prevSelf[s,1:nTrain[s]]-4)));
    GPout[1:nTrain[s]] = rep_vector(1,nTrain[s])./(1 + exp(-(m_out[s]-1)*(prevSelf[s,1:nTrain[s]]-4)));
