...
parameters {
  ...
  vector[nSubjects] shift_pr; // shift
...
transformed parameters {
  ...
  for (i in 1:nSubjects) {
    ...
    shift[i] = Phi_approx(mu_pr[4] + sigma[4] * shift_pr[i]) * 6;
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    GPin[1:nTrain[s]] = rep_vector(1,nTrain[s])./(1 + exp((-m[s])*(prevSelf[s,1:nTrain[s]]- (shift[s]+1) )));
    GPout[1:nTrain[s]] = rep_vector(1,nTrain[s])./(1 + exp((m[s])*(prevSelf[s,1:nTrain[s]]- (shift[s]+1) )));
