...
# Parallel structure to loo_delta_indiff_s1.R
...
# ── 2. Load LOO objects ───────────────────────────────────────────────────────
loo_sym_path  <- here("Fits", "loo_s2_sym_lambda.rds")
loo_asym_path <- here("Fits", "loo_s2_asym_lambda.rds")

if (!file.exists(loo_sym_path) || !file.exists(loo_asym_path)) {
  stop("LOO files not found. Run run_model_comparison_s2.R first.")
}
...
# Determine winning model from LOO comparison
loo_comp <- readRDS(here("Fits","loo_s2_sym_lambda.rds"))  # placeholder; use winning model params
# Try to load sym_lambda params first; fall back to asym_lambda if needed
...
# Individual differences from fullTest
