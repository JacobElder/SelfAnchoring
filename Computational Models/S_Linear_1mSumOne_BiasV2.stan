...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials[s]) {
      ...
      // prob[1] = ( pow( ( simW[1] ) ,tau[s] ) ) / ( ( pow( simW[1] , tau[s] ) ) + ( pow( ( simW[2] + bias[s] ) ,tau[s] ) ) ); // convert to probabilities
      prob[2] = ( pow( ( simW[2] + bias[s] ) ,tau[s] ) ) / ( ( pow( simW[1]  , tau[s] ) ) + ( pow( ( simW[2] + bias[s] ) ,tau[s] ) ) ); // convert to probabilities
      prob[1] = 1 - prob[2];
