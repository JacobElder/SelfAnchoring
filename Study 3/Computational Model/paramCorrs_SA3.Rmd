...
m<-ez::ezANOVA(allParams2, dv = m, wid = subID, between = .(outgroup,racid) )
m
...
Id.ProjRate.Scatter.3 <- ggplot(indDiffs, aes(MGIS,m,color=outgroup)) + geom_point() + geom_smooth(method="lm") + jtools::theme_apa() + scale_color_manual(values = natparks.pals("Charmonix",2)) +
    scale_fill_manual(values = natparks.pals("Charmonix", 2)) + xlab("Social Identification") + ylab("Projection Rate") + theme(legend.position = c(.1,.85))
...
groupDiffs.23
ggsave(plot=groupDiffs.23,filename="/Volumes/Research Project/Self-Anchoring/Combined/Plots/groupDiffsSelfProj.png", width = 6, height = 4.25, units = "in", dpi=300)
```

```{r}
Id.ProjRate.Scatter.123 <- Id.ProjRate.Scatter.1 + Id.ProjRate.Scatter.2 + Id.ProjRate.Scatter.3
Id.ProjRate.Scatter.123
ggsave(plot=Id.ProjRate.Scatter.123,filename="/Volumes/Research Project/Self-Anchoring/Combined/Plots/Id.SelfProj.png", width = 10, height = 6, units = "in", dpi=300)
```
