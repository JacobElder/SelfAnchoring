# Ad-Hoc Exploratory Script for Study 1
# Runs the Symmetric + Lambda + Certainty model.
...
# 1. Load Data (copied from main S1 script)
...
message(paste("Starting Study 1 exploratory model:", model_name))
...
fit$save_object(here("Fits", paste0("fit_s1_certainty_exploratory.rds")))

l <- fit$loo()
saveRDS(l, here("Fits", paste0("loo_s1_certainty_exploratory.rds")))

sum_fit <- fit$summary()
write.csv(sum_fit, here("Results", paste0("summary_s1_certainty_exploratory.csv")), row.names = FALSE)

message("Exploratory run for Study 1 complete.")
