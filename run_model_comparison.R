...
# Setup
# Use 2 cores per model to fit 4 models in parallel on an 8-core machine
options(mc.cores = 2) 
...
message("Fitting Study 1 models in parallel...")
...
results_list <- mclapply(names(models), function(model_name) {
  ...
  # Save Fit
  fit$save_object(paste0("fit_s1_", model_name, ".rds"))
...
write.csv(as.data.frame(comp), "model_comparison_s1_results.csv")
write.csv(do.call(rbind, diag_list), "model_diagnostics_s1.csv")

message("Study 1 complete.")
