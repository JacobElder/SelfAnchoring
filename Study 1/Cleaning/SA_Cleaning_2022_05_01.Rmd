...
```{r}
setwd("/Volumes/Research Project/Self-Anchoring/Study 1/Cleaning/input/")
...
```{r}
postQs <- read.csv("/Volumes/Research Project/Self-Anchoring/Study 1/Cleaning/Qualtrics Data/SA1postQs.csv")
preQs <- read.csv("/Volumes/Research Project/Self-Anchoring/Study 1/Cleaning/Qualtrics Data/SA1preQs.csv")
...
write.csv(indDiff, "/Volumes/Research Project/Self-Anchoring/Study 1/Cleaning/Qualtrics Data/CleanSA1Qs.csv")
```
...
sketchMat$remove <- ifelse(sketchMat$sketch > 0 | sketchMat$ConDeb == 2 | sketchMat$Usable == 2 | sketchMat$Serious < 4, 1, 0)
sketchMat$remove[is.na(sketchMat$remove)] <- 2
```

```{r}
table(sketchMat$sketch)
table(sketchMat$ConDeb)
table(sketchMat$Usable)
table(sketchMat$Serious)
table(sketchMat$decep)
...
```{r}
for(i in uSubs){
  r <- unique(fullTest$Estimator[fullTest$subID==i])
  for(j in unique(fullTest$Idx)){
    ...
    # fullTest$propCorrLOO[fullTest$subID==i & fullTest$Idx==j] <- mean(fullTest$optionChoiceN[fullTest$subID!=i & fullTest$Idx==j & fullTest$Estimator==r], na.rm=T)
...
write.csv(fullTrain, "./output/fullTrain.csv", row.names = F)
