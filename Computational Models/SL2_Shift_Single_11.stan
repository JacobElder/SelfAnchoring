...
transformed parameters {
  ...
  shift = mu_pr[3] + sigma[3] * shift_pr;
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials[s]) {
      ...
      prob[2] = (1)/(1 + exp((-(m[s]-10) )*( (( sum(prevSelf[s,1:nTrain[s]].*PS)/sum(prevSelf[s,1:nTrain[s]]) ) )  - (shift[s]) ) )) ;
      #print(prob)
      #print("Probabilities")
      #print(prob)
      prob[1] = 1 - prob[2];
...
generated quantities {
  ...
  { // local section, this saves time and space
    ...
    for (t in 1:nTrials[s]) {
      ...
      //prob[2] = (1)/(1 + exp((-(m[s]-10) )*( ( dot_product(prevSelf[s,1:nTrain[s]],PS) / sum(prevSelf[s,1:nTrain[s]]) )   - (shift[s]) ) )) ;
      prob[2] = (1)/(1 + exp((-(m[s]-10) )*( (( sum(prevSelf[s,1:nTrain[s]].*PS)/sum(prevSelf[s,1:nTrain[s]]) ) )  - (shift[s]) ) )) ;
      //prob[2] = (1)/(1 + exp((-(m[s]-10) )*( ( dot_product(prevSelf[s,1:nTrain[s]] - shift[s],PS) ) )  )) ;
      prob[1] = 1 - prob[2];
