...
#fullTrain <- indDiffs %>% select(subID, groupHomoph, seHomoph, SStatus:IdImp) %>% full_join(fullTrain, by="subID")
```
...
fullTrain$outgroup <- as.factor(fullTrain$outgroup)
fullTrain$outgroup <- relevel(fullTrain$outgroup,"Not UCR")
...
```{r}
m <- glmer( ingChoiceN ~ selfResp.Z * outgroup  +  ( selfResp.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
m <- glmer( ingChoiceN ~ WSR.Z * outgroupE + ( WSR.Z | subID) + ( outgroupE | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
    ...
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("WSR.Z","outgroupE")) %>% plot()
```
...
## Zero-Order Effect, depending on Outgroup Comparison (Ordered from Negation-Low-High)

```{r}
fullTest$outgroupOrder <- factor(fullTest$outgroup, levels = c("Not UCR", "CSU LA", "UCLA"), ordered = T)
m <- glmer( ingChoiceN ~ WSR.Z * outgroupOrder + ( WSR.Z | subID) + ( outgroupOrder | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("WSR.Z","outgroupOrder")) %>% plot()
```
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * UCR_Status + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * IdImp + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(RSE) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(NTB) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(MGIS) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(SING.Inter) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(Ent) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(Proto) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(NFC) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * outgroup * scale(GSim) + ( WSR.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","NFC","outgroup")) %>% plot()
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * novel * outgroup + ( WSR.Z + novel | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS) + scale(outGsimS) + (  scale(inGsimS) + scale(outGsimS) | subID) + (1 | trait), data = fullTrain)
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup + scale(outGsimS)+outgroup + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```
...
UCLA slope is steeper for people who UCR identity is important

```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*IdImp + scale(outGsimS)*outgroup*IdImp + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","IdImp")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*SCC + scale(outGsimS)*outgroup*SCC + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","SCC")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*MGIS + scale(outGsimS)*outgroup*MGIS + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","MGIS")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*Ent + scale(outGsimS)*outgroup*Ent + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","Ent")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```

#### Self-Prototypicality

Effect of ingroup sim steeper for CSU LA for those lower in self-prototypicality of group

Effect of outgroup sim steeper for Not UCR for those higher in self-prototypicality of group

```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*Proto + scale(outGsimS)*outgroup*Proto + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","Proto")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
ggpredict(m, c("outGsimS","outgroup","Proto")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```

#### Need to Belong

Effect of ingroup sim steeper for CSU LA for those lower in Need to Belong

```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*NTB + scale(outGsimS)*outgroup*NTB + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","NTB")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
ggpredict(m, c("outGsimS","outgroup","NTB")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```

#### Need for Cognition

Effect of ingroup sim much steeper for UCLA for those lower in Need for Cognition, and much steeper for CSU LA for those higher in Need for Cognition

Effect of outgroup sim for UCLA more positive for those higher in Need for Cognition and more negative for Not UCR for those higher in Need for Cognition

```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*NFC + scale(outGsimS)*outgroup*NFC + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","NFC")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
ggpredict(m, c("outGsimS","outgroup","NFC")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```

#### Rosenberg Self-Esteem

```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*RSE + scale(outGsimS)*outgroup*RSE + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","RSE")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
ggpredict(m, c("outGsimS","outgroup","RSE")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```

#### Dialectical Self

Effect of outgroup sim for Not UCR steeper for those lower in dialectical self

```{r}
m <- lmer( scale(selfResp) ~ scale(inGsimS)*outgroup*DS + scale(outGsimS)*outgroup*DS + (  scale(inGsimS) + scale(outGsimS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsimS","outgroup","DS")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
ggpredict(m, c("outGsimS","outgroup","DS")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```

## Metacontrast Ratio Moderated by Condition

```{r}
fullTrain$metaContrastS[is.infinite(fullTrain$metaContrastS)] <- NA
fullTrain$metaContrastS <- log(fullTrain$metaContrastS)
m <- lmer( scale(selfResp) ~ scale(metaContrastS)*outgroup*IdImp.x + (  scale(metaContrastS) | subID) + ( 1 | trait), data = fullTrain)
```


```{r}
fullTrain$metaContrastS <- fullTrain$inGsimS/fullTrain$outGsimS
m <- lmer( scale(selfResp) ~ scale(inGsimS) + outgroup + scale(desirability) + ( scale(metaContrastS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("metaContrastS","outgroup")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()

fullTrain$outgroupOrder <- factor(fullTrain$outgroup, levels = c("Not UCR", "CSU LA", "UCLA"), ordered = T)
m <- lmer( scale(selfResp) ~ scale(metaContrastS)*outgroupOrder + (  scale(inGsim) + scale(metaContrastS) | subID) + ( 1 | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
...
m <- glmer( ingChoiceN ~ WSR.Z * SE + ( WSR.Z | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
    ...
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("WSR.Z","SE")) %>% plot(show.title=F)
```
