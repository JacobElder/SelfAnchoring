...
```{r}
load("/Volumes/Research Project/Self-Anchoring/Combined/Combined/reportingAnalysesEnv_2023_05_19.RData")
save.image("/Volumes/Research Project/Self-Anchoring/Combined/Combined/reportingAnalysesEnv_2023_05_19.RData")
```
...
```{r}
WSR.ING.COV <- brm(ingChoiceN ~ WSR.Z + propCorrLOO.Z + desirability.Z + ( WSR.Z + propCorrLOO.Z | subID) + ( 1 | trait), data=fullTest1, cores=ncores, family = bernoulli(link = "logit") )
sexit(WSR.ING.COV) %>% as_tibble %>% mutate(OR = exp(Median), p = 2 * (1-Direction)) %>% select(-CI) %>% mutate_if(is.numeric, round, 3) %>% kbl(caption = "Study 1: Similarity-to-Self Main Effect") %>% kable_classic(full_width = F, html_font = "Cambria")
```
...
fullTest2$propCorrInLOO.Z <- as.numeric(scale(fullTest2$propCorrInLOO)[,1])
...
```{r}
WSR.ING.2.COV <- brm(ingChoiceN ~ WSR.Z + propCorrInLOO.Z + desirability.Z + ( WSR.Z + propCorrInLOO.Z | subID) + ( 1 | trait), data=fullTest2, cores=ncores, family = bernoulli(link = "logit") )
sexit(WSR.ING.2.COV) %>% as_tibble %>% mutate(OR = exp(Median), p = 2 * (1-Direction)) %>% select(-CI) %>% mutate_if(is.numeric, round, 3) %>% kbl(caption = "Study 2: Similarity-to-Self Main Effect") %>% kable_classic(full_width = F, html_font = "Cambria")
```
...
```{r}
WSR.ING.COND.2test <- brm( ingChoiceN ~ WSR.Z * outgroup + desirability.Z + evalLOO.Z + (  WSR.Z + desirability.Z + evalLOO.Z | subID) + ( outgroup | trait), data=fullTest2, cores=ncores, family = bernoulli(link = "logit") )
...
```{r}
WSR.ING.COND.2test <- brm( ingChoiceN ~ WSR.Z * outgroup + desirability.Z + scale(RT) + novel + ( WSR.Z + desirability.Z + scale(RT) + novel | subID) + ( outgroup | trait), data=fullTest2, cores=ncores, family = bernoulli(link = "logit") )
sexit(WSR.ING.COND.2test) %>% as_tibble %>% mutate(OR = exp(Median), p = 2 * (1-Direction)) %>% kable()
ggpredict(WSR.ING.COND.2test, c("WSR.Z", "outgroup")) %>% plot()
```
...
```{r}
WSR.ING.COND.2.COV <- brm(ingChoiceN ~ WSR.Z * outgroup + propCorrInLOO.Z + desirability.Z + ( WSR.Z + propCorrInLOO.Z | subID) + ( 1 | trait), data=fullTest2, cores=ncores, family = bernoulli(link = "logit") )
sexit(WSR.ING.COND.2.COV) %>% as_tibble %>% mutate(OR = exp(Median), p = 2 * (1-Direction)) %>% select(-CI) %>% mutate_if(is.numeric, round, 3) %>% kbl(caption = "Study 2: Similarity-to-Self Interaction") %>% kable_classic(full_width = F, html_font = "Cambria")
ggpredict(WSR.ING.COND.2.COV, c("WSR.Z", "outgroup")) %>% plot()
```
...
```{r}
m <- lmer( scale(selfResp) ~ outGsimS.Z * IdImp + inGsimS.Z * IdImp  + (  metaContrastS.Z | subID) + (1 | trait), data = fullTrain2)
```
...
indDiffs3 <- indDiffs3 %>% mutate(inWarm = case_when(
  racid == "Asian" ~ Therm_2,
...
otherWarm = case_when(
  ...
  mutate(inOutTherm = inWarm - outWarm,
             inOtherTherm = inWarm - otherWarm) %>%
  mutate(inOutStatus = case_when(
      condition == "Majority" ~ inMajorityStatus,
      condition == "Minority" ~ inMinorityStatus,
  )
         )

fullTrain3 <- indDiffs3 %>% select(subID, inWarm, outWarm, otherWarm, inOutTherm, inOtherTherm, inOutStatus, groupHomoph) %>% full_join(fullTrain3, by = "subID")
...
fullTrain3 <- fullTest3 %>% select(subID, trait, propCorrInLOO) %>% right_join(fullTrain3, by = c("subID","trait"))
```
...
fullTest3$propCorrInLOO.Z <- as.numeric(scale(fullTest3$propCorrInLOO)[,1])
...
```{r}
WSR.ING.3.COV <- brm(ingChoiceN ~ WSR.Z + propCorrInLOO.Z + desirability.Z + ( WSR.Z + propCorrInLOO.Z | subID) + ( 1 | trait), data=fullTest3, cores=ncores, family = bernoulli(link = "logit") )
sexit(WSR.ING.3.COV) %>% as_tibble %>% mutate(OR = exp(Median), p = 2 * (1-Direction)) %>% select(-CI) %>% mutate_if(is.numeric, round, 3) %>% kbl(caption = "Study 3: Similarity-to-Self Main Effect") %>% kable_classic(full_width = F, html_font = "Cambria")
```
...
```{r}
WSR.ING.COND.3.COV <- brm(ingChoiceN ~ WSR.Z * condition + propCorrInLOO.Z + desirability.Z + ( WSR.Z + propCorrInLOO.Z | subID) + ( condition | trait), data=fullTest3, cores=ncores, family = bernoulli(link = "logit") )
sexit(WSR.ING.COND.3.COV) %>% as_tibble %>% mutate(OR = exp(Median), p = 2 * (1-Direction)) %>% select(-CI) %>% mutate_if(is.numeric, round, 3) %>% kbl(caption = "Study 3: Similarity-to-Self Main Effect") %>% kable_classic(full_width = F, html_font = "Cambria")
ggpredict(WSR.ING.COND.3.COV, c("WSR.Z", "condition")) %>% plot()
```
...
```{r}
m <- lmer( scale(selfResp) ~ outGsimS.Z * IdImp + inGsimS.Z * IdImp  + (  metaContrastS.Z | subID) + (1 | trait), data = fullTrain3)
```
...
```{r}
m <- lmer( scale(selfResp) ~ inGsimS.Z * condition + outGsimS.Z * condition + scale(propCorrInLOO) + (  1 | subID) + ( 1 | trait), data = fullTrain3)
summary(m)
```
