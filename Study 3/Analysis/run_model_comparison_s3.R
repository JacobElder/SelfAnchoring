...
fit_and_save <- function(model_name) {
  ...
  write.csv(sum_fit, here("Results", paste0("summary_s3_", model_name, ".csv")), row.names = FALSE)
  
  ind_params <- sum_fit %>%
    filter(str_detect(variable, "\\[")) %>%
    select(variable, median, rhat, ess_bulk)
  write.csv(ind_params, here("Results", paste0("params_ind_s3_", model_name, ".csv")), row.names = FALSE)
  ...
  write.csv(diag_df, here("Results", paste0("diag_s3_", model_name, ".csv")), row.names = FALSE)
...
message("All models fit for Study 3. Comparing results from saved files.")
loo_files <- list.files(path = here("Fits"), pattern = "loo_s3_.*\\.rds", full.names = TRUE)
diag_files <- list.files(path = here("Results"), pattern = "diag_s3_.*\\.csv", full.names = TRUE)
...
if (length(loo_files) >= length(models) && length(diag_files) >= length(models)) {
  ...
  names(loo_list) <- str_remove(str_remove(basename(loo_files), "loo_s3_"), ".rds")
  
  diag_list <- lapply(diag_files, read.csv)
  
  comp <- loo_compare(loo_list)
  write.csv(as.data.frame(comp), here("Results", "model_comparison_s3_results.csv"), row.names = FALSE)
  write.csv(do.call(rbind, diag_list), here("Results", "model_diagnostics_s3.csv"), row.names = FALSE)
  
  message("Study 3 comparison complete.")
} else {
  warning("Could not find all necessary loo/diagnostic files for S3 comparison.")
}
