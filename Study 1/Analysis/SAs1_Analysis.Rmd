...
```{r}
indDiffs <- merge(indDiffs, biasanchor2params, by="subID")
indDiffs %>% select(groupHomoph, SAo, DS:SING.Ind) %>% corToOne(., "SAo")
indDiffs %>% select(groupHomoph, SAo, DS:SING.Ind) %>% plotCorToOne(., "SAo")
```
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * novel + ( predicted.Z + novel | subID) + (1 | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
```
