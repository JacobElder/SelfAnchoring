...
parameters {
  ...
  vector[nSubjects] shift_pr;  // slope for L
...
transformed parameters {
  ...
  for (i in 1:nSubjects) {
    ...
    shift[i] = Phi_approx(mu_pr[5] + sigma[5] * shift_pr[i]);
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    GPin[1:nTrain[s]] = rep_vector(L[s],nTrain[s])./(1 + exp((-m[s])*(prevSelf[s,1:nTrain[s]]- (4 + ( shift[s] - 2) ) ))) + ((1-L[s])/2);
    GPout[1:nTrain[s]] = rep_vector(L[s],nTrain[s])./(1 + exp((m[s])*(prevSelf[s,1:nTrain[s]]- (4 - ( shift[s] - 2) ) ))) + ((1-L[s])/2);
