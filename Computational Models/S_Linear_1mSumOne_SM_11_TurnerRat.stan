data {
  ...
  real inGsim[nSubjects, maxTrials]; // self-weighted similarity
  real outGsim[nSubjects, maxTrials]; // self-weighted similarity
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials[s]) {
      ...
      curW[1] = (wOut[s]-1) * outGsim[s,t];
      curW[2] = (wIn[s]-1) * inGsim[s,t];
