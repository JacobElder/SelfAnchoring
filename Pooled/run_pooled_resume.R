# Pooled Model Comparison — Resume Script
# Runs only sym_lambda and asym_lambda (bias + symmetric already complete).
# Loads all 4 LOO files and compares at the end.
...
# Run only the remaining models
for (m in names(models_remaining)) {
  fit_and_save_pooled(m)
}

# Compare all 4 models (including already-saved bias + symmetric)
message("All models fit. Comparing all 4 pooled models.")
...
diag_files <- lapply(all_model_names, function(m) {
  f <- here("Results", paste0("diag_pooled_", m, ".csv"))
  if (file.exists(f)) read.csv(f) else NULL
})
diag_list <- Filter(Negate(is.null), diag_files)
