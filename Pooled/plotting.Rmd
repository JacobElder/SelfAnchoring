...
pacman::p_load("lmerTest", "ggeffects","r2glmm", "tidyverse","here", "sjPlot", "ggpubr", "wesanderson", "effectsize","broom.mixed","corrr","report", "ez", "ggdist","tidyverse")
...
Sim.Condition.Plot1 <-ggplot(p, aes(x, predicted)) +  geom_line() + geom_ribbon(aes(ymin=conf.low, ymax=conf.high), alpha=0.15) + theme(axis.text=element_text(size=10),
...
ggsave(plot=Sim.Condition.PlotComb, filename="/Volumes/Research Project/Self-Anchoring/Combined/Plots/CombinedPlot.png", width=6, height=10, dpi=400)
```

```{r}
threePlots <- function(plot1, plot2, plot3, xaxis, yaxis){
  
  arranged <- ggarrange(plot1 + rremove("ylab") + rremove("xlab"), plot2 + rremove("ylab") + rremove("xlab"), plot3 + rremove("ylab") + rremove("xlab"), ncol = 3, nrow = 1, hjust=.42)
  addedAxes <- annotate_figure(arranged, left = textGrob(yaxis, rot = 90, vjust = 1, hjust = .4, gp = gpar(cex = 1.3, fontsize=8)),
                               bottom = textGrob(xaxis, vjust = -.35, hjust = .33, gp = gpar(cex = 1.3, fontsize=8))
  )
  return(addedAxes)
  
}
Sim.Condition.PlotComb <- threePlots(Sim.Condition.Plot1, Sim.Condition.Plot2, Sim.Condition.Plot3, "Similarity-to-Self","Prob. of Ingroup Classification")
#Sim.Condition.PlotComb <- Sim.Condition.PlotComb +
#  theme(plot.margin = margin(1.2,1.2,1.2,1.2, "in"))

library(patchwork)
Sim.Condition.PlotComb <- Sim.Condition.Plot1 + Sim.Condition.Plot2 + Sim.Condition.Plot3
ggsave(plot=Sim.Condition.PlotComb, filename="/Volumes/Research Project/Self-Anchoring/Combined/Plots/CombinedPlot.png", width=10, height=6, dpi=400)
```
