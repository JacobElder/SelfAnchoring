...
```{r}
SR.ING.COND.3 <-brm( ingChoiceN ~ selfResp.Z * condition + ( selfResp.Z | subID) + ( condition | trait), data=fullTest2, cores=ncores, family = bernoulli(link = "logit") )
...
```{r}
ISOS.SR.COND.3 <-brm( selfResp ~ inGsimS.Z * outgroup + outGsimS.Z * condition + (  inGsimS.Z + outGsimS.Z | subID) + ( condition | trait), data = fullTrain3, cores=ncores, family = cumulative(link = "logit") )
