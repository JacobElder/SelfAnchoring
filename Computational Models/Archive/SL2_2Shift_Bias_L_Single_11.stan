...
transformed parameters {
  ...
  for (i in 1:nSubjects) {
    ...
    bias[i] = Phi_approx(mu_pr[5] + sigma[5] * bias_pr[i]);
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials[s]) {
      ...
      prob[2] = (L[s])/(1 + exp((-(m[s]-10) )*(dot_product(prevSelf[s,1:nTrain[s]],PS) -  (shift[s]) ) ));
      prob[1] = 1 - prob[2];

      prob[2] = ( (bias[s]) * pow( ( prob[2] ) ,tau[s] ) ) / ( ( (1 - bias[s]) *  pow( prob[1]  , tau[s] ) ) + ( (bias[s]) * pow( ( prob[2] ) ,tau[s] ) ) ); // convert to probabilities
      prob[1] = 1 - prob[2];
