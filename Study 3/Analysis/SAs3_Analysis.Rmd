...
here::i_am("Study 3/Analysis/SAs3_Analysis.Rmd")
```
...
#indDiffs <- indDiffs[indDiffs$Condition!="",]
...
statusDf <- pivot_longer(indDiffs, cols=ends_with("_Status"), names_to="Group", values_to="Status") %>% select(subID, Group, Status) %>% drop_na()
...
```{r}
warmthDf <- pivot_longer(indDiffs, cols=starts_with("Therm_"), names_to="Group", values_to="Warmth") %>% select(subID, Group, Warmth) %>% drop_na()
...
indDiffs %>% select(groupHomoph, seHomoph, SStatus, SCC:IdImp) %>% corToOne(., "groupHomoph")
indDiffs %>% select(groupHomoph, seHomoph, SStatus, SCC:IdImp) %>% plotCorToOne(., "groupHomoph")
```
...
```{r}
library(ez)
m<-ezANOVA(indDiffs[!is.na(indDiffs$groupHomoph),], dv=groupHomoph, wid=subID, between=condition)
hsd <- TukeyHSD(aov(groupHomoph ~ condition, data=indDiffs[!is.na(indDiffs$groupHomoph),]))
print(m)
print(hsd)
ezPlot(indDiffs[!is.na(indDiffs$groupHomoph),], groupHomoph, wid=subID, between=condition, x=.(condition))

indDiffs$groupHomoph.Z <- scale(indDiffs$groupHomoph)
m <- lm(groupHomoph.Z ~ condition, data=indDiffs)
summary(m)
ggpredict(m, c("condition")) %>% plot(show.title=F, add.data=T)

ggplot(indDiffs, aes(condition, groupHomoph.Z)) + 
  ggdist::stat_halfeye(adjust = .5, width = .7, .width = 0, justification = -.2, point_colour = NA) + 
  geom_boxplot(width = .2, outlier.shape = NA) + 
  geom_jitter(width = .05, alpha = .3) + labs(y="Homophily",x="Racial Group Comparison") + jtools::theme_apa()
```
...
Slightly steeper classification of ingroup as desirable when contrasted against majority vs. minority

```{r}
m <- glmer( ingChoiceN ~ desirability.Z * condition +  ( desirability.Z  | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("desirability.Z","condition")) %>% plot(show.title=F) + xlab("Desirability") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
```{r}
m <- glmer( ingChoiceN ~ selfResp.Z * condition + desirability.Z +  ( selfResp.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("selfResp.Z", "condition")) %>% plot(show.title=F) + xlab("Self-Descriptiveness") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
p <- ggpredict(m, c("WSR.Z","condition"))
Sim.Condition.Plot <-ggplot(p, aes(x, predicted)) +  geom_line(aes(linetype=group, color=group)) + geom_ribbon(aes(ymin=conf.low, ymax=conf.high, fill=group), alpha=0.15) + scale_linetype_discrete(labels = c("Minority","Majority")) + scale_color_manual(labels = c("Minority","Majority"), values = wes_palette("Darjeeling1")) + scale_fill_manual( 
...
```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("WSR.Z", "condition")) %>% plot(show.title=F) + xlab("Similarity-Weighted Self-Evaluations") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
### Individual Differences for Similarity-to-Self Condition Differences

#### Identity Importance

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * IdImp + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","IdImp","condition")) %>% plot()
```

#### Self-Esteem

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(RSE) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","RSE","condition")) %>% plot()
```

### Need to Belong

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(NTB) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","NTB","condition")) %>% plot()
```

## Multidimensional Ingroup Identification

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(MGIS) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","MGIS","condition")) %>% plot()
```

## Interdependent Self-Construal

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(SING.Inter) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","SING.Inter","condition")) %>% plot()
```

## Entitativity

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(Ent) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","Ent","condition")) %>% plot()
```

## Self-Prototypicality

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(Proto) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","Proto","condition")) %>% plot()
```

## Need for Cognition

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(NFC) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","NFC","condition")) %>% plot()
```

## Group Similarity

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(GSim) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","GSim","condition")) %>% plot()
```

## Group Homophily

```{r}
m <- glmer( ingChoiceN ~ WSR.Z * condition * scale(groupHomoph) + desirability.Z + evalLOO.Z + ( WSR.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
summary(m)
ggpredict(m, c("WSR.Z","groupHomoph","condition")) %>% plot()
```
...
```{r}
m <- glmer( ingChoiceN ~ predicted.Z*condition + desirability.Z + ( predicted.Z | subID) + ( outgroup | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("predicted.Z", "condition")) %>% plot(show.title=F) + xlab("Cross-Validated Expectations") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
```{r}
m <- glmer( ingChoiceN ~ er.Z*condition + desirability.Z + ( er.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("er.Z", "condition")) %>% plot(show.title=F) + xlab("Dot-Product Expected Rating") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
```{r}
m <- glmer( ingChoiceN ~ slope.Z*condition + desirability.Z + ( slope.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("slope.Z", "condition")) %>% plot(show.title=F) + xlab("Similarity-Weighted Self-Evaluations") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
```{r}
m <- glmer( ingChoiceN ~ neighAveInSE.Z*condition + desirability.Z + ( neighAveInSE.Z | subID) + ( condition | trait), data = fullTest, family = binomial, control = glmerControl(optimizer = "bobyqa",
                                    optCtrl = list(maxfun = 100000)),
    nAGQ = 1)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "OR", string.se = "SE", string.stat = "z", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("neighAveInSE.Z", "condition")) %>% plot(show.title=F) + xlab("Inwards Neighbors' Average Self-Descriptiveness") + ylab("Likelihood of Ingroup Choice") + jtools::theme_apa()
```
...
```{r}
m <- lmer( scale(selfResp) ~ scale(inGsim)*condition + scale(outGsim)*condition + (  scale(inGsim) + scale(outGsim) | subID) + ( condition | trait), data = fullTrain)
tab_model(m, collapse.ci = T, show.stat=T, show.r2 = T, show.se = T, string.pred = c("Fixed Effects"), string.est = "B", string.se = "SE", string.stat = "t", digits = 3, emph.p = F, dv.labels = "Ingroup Prediction")
ggpredict(m, c("inGsim","condition")) %>% plot(show.title=F) + xlab("Similarity to Ingroup Choices") + ylab("Self-Evaluation") + jtools::theme_apa()
```
