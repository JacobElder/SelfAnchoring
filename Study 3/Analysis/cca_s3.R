# cca_s3.R
# Canonical Correlation Analysis: model parameters × personality (Study 3)
# Parallel to Study 1/Analysis/cca_s1.R and Study 2/Analysis/cca_s2.R
#
...
# Analysis is run across ALL conditions; condition is indicated by color in biplot.
#
# Prerequisites:
#   - ind_diffs_s3_full_enriched.csv (from param_comparison_conditions_s3.R)
#     OR ind_diffs_s3_full.csv (from loo_delta_indiff_s3.R) as fallback
#
# Output:
#   Results/cca_s3_biplot.png
#   Results/cca_s3_correlations.csv
#   Results/cca_s3_loadings.csv
...
enriched_file <- here("Results", "ind_diffs_s3_full_enriched.csv")
base_file     <- here("Results", "ind_diffs_s3_full.csv")
...
ind <- if (file.exists(enriched_file)) {
  ...
  stop("No ind_diffs file found. Run loo_delta_indiff_s3.R first.")
}

if (!"condition" %in% names(ind)) {
  ...
  ind <- left_join(ind, fulldf_cond, by = "subID")
}

message(sprintf("N subjects: %d", nrow(ind)))
cat("N by condition:\n"); print(table(ind$condition))

# ── Load groupHomoph from fullTest ─────────────────────────────────────────────
ft <- read.csv(here("Study 3/Cleaning/output/fullTest_fixed.csv")) |>
  filter(!is.na(ingChoiceN)) |>
  group_by(subID) |>
  slice(1) |>
  ungroup() |>
...
write.csv(cv_res,                          here("Results", "cca_s3_correlations.csv"), row.names = FALSE)
write.csv(bind_rows(load_X_df, load_Y_df), here("Results", "cca_s3_loadings.csv"),     row.names = FALSE)
...
p_biplot <- ggplot(ld, aes(x = CV1, y = CV2, colour = set, label = label)) +
  ...
  scale_colour_manual(values = c("Model (X)" = "#2B5C8A", "Personality (Y)" = "#CC79A7"),
                      name = NULL) +
  labs(
    title    = sprintf("CCA Biplot — Study 3, Racial Groups (N = %d)", nrow(df_cc)),
...
ggsave(here("Results", "cca_s3_biplot.png"), p_biplot, width = 8, height = 7, dpi = 200)

message("Saved: Results/cca_s3_biplot.png")
message("Saved: Results/cca_s3_correlations.csv, Results/cca_s3_loadings.csv")
message(sprintf("\nNote: N = %d; %d + %d = %d variables. CCA exploratory.",
