...
```{r}
setwd("/Volumes/Research Project/Self-Anchoring/Study 3/Cleaning/input/")
...
Outgroups <- mergedRaw %>% 
  select(subID, racid, racid2) %>%
  group_by(subID) %>%
  filter(row_number()==n()) %>%
  rename(ingroup = racid, outgroup = racid2) 

Outgroups$condition <-
  case_when(
  Outgroups$outgroup == "White" ~ "Majority",
  Outgroups$outgroup == "Latino" ~ "Minority",
  Outgroups$outgroup == "Asian" ~ "Minority"
)
...
```{r}
postQs <- read.csv("/Volumes/Research Project/Self-Anchoring/Study 3/Cleaning/Qualtrics Data/SA3postQs.csv")
preQs <- read.csv("/Volumes/Research Project/Self-Anchoring/Study 3/Cleaning/Qualtrics Data/SA3preQs.csv")
preQs <- preQs[!is.na(preQs$id),]
postQs <- postQs[!is.na(postQs$id),]
postQs <- subset(postQs, id != "' expInfo['participant'] '")
postQs$id <- as.numeric(postQs$id)
preQs <- subset(preQs, id != "47001")
preQs <- subset(preQs, id > 100)
...
write.csv(indDiff, "/Volumes/Research Project/Self-Anchoring/Study 3/Cleaning/Qualtrics Data/CleanSA3Qs.csv")
```
...
```{r}
indDiff$WhiteAsianTherm <- indDiff$Therm_1 - indDiff$Therm_2
indDiff$WhiteLatinoTherm <- indDiff$Therm_1 - indDiff$Therm_4
indDiff$AsianLatinoTherm <- indDiff$Therm_2 - indDiff$Therm_4

indDiff$inMinorityTherm <- ifelse(indDiff$racid=="Asian", indDiff$AsianLatinoTherm, -indDiff$AsianLatinoTherm)
indDiff$inMajorityTherm <- ifelse(indDiff$racid=="Asian", -indDiff$WhiteAsianTherm, -indDiff$WhiteLatinoTherm)

indDiff$WhiteAsianStatus <- indDiff$White_Status - indDiff$Asian_Status
indDiff$WhiteLatinoStatus <- indDiff$White_Status - indDiff$Latino_Status
indDiff$AsianLatinoStatus <- indDiff$Asian_Status - indDiff$Latino_Status

indDiff$inMinorityStatus <- ifelse(indDiff$racid=="Asian", indDiff$AsianLatinoStatus, -indDiff$AsianLatinoStatus)
indDiff$inMajorityStatus <- ifelse(indDiff$racid=="Asian", -indDiff$WhiteAsianStatus, -indDiff$WhiteLatinoStatus)
```
...
sketchMat$remove[is.na(sketchMat$remove)] <- 2
```


```{r}
table(sketchMat$sketch)
table(sketchMat$Usable)
table(sketchMat$Serious)

table(sketchMat$remove)

total <- length(unique(mergeTest$subID))
```
...
paste0(unique(mergeTest$subID))
cleanTest <- mergeTest[mergeTest$subID %in% sketchMat$subID[sketchMat$remove!=1],]


paste0("Removed ", total-length(unique(cleanTest$subID)))
```
...
for(i in uSubs){
  ...
  cleanTrain$outGsim[cleanTrain$subID==i & cleanTrain$trialTotalT1==trialNumT1] <- mean(simMat[trainSub$Idx[t],testSub$Idx[which(testSub$ingChoiceN==0)]])
    ...
    cleanTrain$inGsimS[cleanTrain$subID==i & cleanTrain$trialTotalT1==trialNumT1] <- sum(simMat[trainSub$Idx[t],testSub$Idx[which(testSub$ingChoiceN==1)]])
  cleanTrain$outGsimS[cleanTrain$subID==i & cleanTrain$trialTotalT1==trialNumT1] <- sum(simMat[trainSub$Idx[t],testSub$Idx[which(testSub$ingChoiceN==0)]])
...
cleanTrain$metaContrast <- cleanTrain$inGsim / cleanTrain$outGsim
cleanTrain$metaContrastS <- cleanTrain$inGsimS / cleanTrain$outGsimS
cleanTrain$metaContrast[is.infinite(cleanTrain$metaContrast)] <- NA
cleanTrain$metaContrastS[is.infinite(cleanTrain$metaContrastS)] <- NA
```
...
indDiffsclean$outgroup <- NULL
fullTest <- cleanTest %>% inner_join(indDiffsclean, by = "subID")
fullTrain <- cleanTrain %>% inner_join(indDiffsclean, by = "subID")
```

```{r}
fullTest$optionChoiceN <- ifelse(fullTest$ingChoice=="Ingroup", 1, 0)
traitFreqOverUnder <- Rmisc::summarySE(fullTest, measurevar="optionChoiceN", groupvars = c("condition","trait"), na.rm = T)
```
...
```{r}
for(i in uSubs){
  og <- unique(fullTest$condition[fullTest$subID==i])
  r <- unique(fullTest$racid[fullTest$subID==i])
  for(j in unique(fullTest$Idx)){
    
    if(fullTest$condition[fullTest$subID==i & fullTest$Idx==j & fullTest$condition == og]=="Majority"){
      
      fullTest$propCorrOutLOO[fullTest$subID==i & fullTest$Idx==j & fullTest$condition == og] <- 1 - mean(fullTest$optionChoiceN[fullTest$subID!=i & fullTest$Idx==j & fullTest$condition == og], na.rm=T)
      
    }else if(fullTest$condition[fullTest$subID==i & fullTest$Idx==j & fullTest$condition == og]=="Minority"){
      
      fullTest$propCorrOutLOO[fullTest$subID==i & fullTest$Idx==j & fullTest$condition == og] <- 1- mean(fullTest$optionChoiceN[fullTest$subID!=i & fullTest$Idx==j & fullTest$condition == og], na.rm=T)
      
    }
    
    fullTest$propCorrInLOO[fullTest$subID==i & fullTest$Idx==j] <- mean(fullTest$optionChoiceN[fullTest$subID!=i & fullTest$Idx==j & fullTest$racid == r], na.rm=T)
...
```{r}
stereo <- read.csv("./output/stereoData.csv")
stereo <- stereo %>% select(trait, asianStereo, latinoStereo)
fullTrain <- merge(fullTrain, stereo, by = "trait", all.x=T)
fullTest <- merge(fullTest, stereo, by = "trait", all.x=T)
fullTrain$stereo <- ifelse(fullTrain$racid=="Latino", fullTrain$latinoStereo, fullTrain$asianStereo)
fullTest$stereo <- ifelse(fullTest$racid=="Latino", fullTest$latinoStereo, fullTest$asianStereo)
```
...
arrow::write_parquet(fullTrain, "./output/fullTrain.parquet")
